...
void EventDisplay::Begin(TTree * /*tree*/) {
  ...
  for (int iThresh = 1; iThresh < 4; ++iThresh) {
    ...
    ssName << "ij_" << iThresh;
    auto ijTH2 = new TH2F(ssName.str().c_str(), "IvJ Display ", 96, 0, 95, 96, 0, 95);
    ssName.str(std::string());
    ssName << "ik_" << iThresh;
    auto ikTH2 = new TH2F(ssName.str().c_str(), "IvK Display ", 50, 0, 50, 96, 0, 95);
    ssName.str(std::string());
    ssName << "jk_" << iThresh;
    auto jkTH2 = new TH2F(ssName.str().c_str(), "JvK Display ", 50, 0, 50, 96, 0, 95);
...
Bool_t EventDisplay::Process(Long64_t entry) {
  ...
  std::cout << " Evt: " << *EventNumber << "\tnHit: " << *NumberOfHits << "\t " << *NumberOfCerenkov1Hits << " "
            << *NumberOfCerenkov2Hits << " " << *NumberOfCerenkov3Hits << " Th : " << *CerThreshold
            << " Asic : " << *CerAsic << " Chan : " << *CerChan << " TooMany : " << *TriggerHasTooManyCerenkov
            << " CerTime : " << *CerenkovTime << std::endl;
  ...
  for (int iThresh = 1; iThresh < 4; ++iThresh) {
    ...
    m_mThreshTH2.at(iThresh).at("ik")->SetMarkerSize(.25);
